...
def create_folder_structure(base_folder):
    os.makedirs(os.path.join(base_folder, 'created', 'sparse'), exist_ok=True)
    os.makedirs(os.path.join(base_folder, 'SLAM_images', 'images'), exist_ok=True)
    os.makedirs(os.path.join(base_folder, 'triangulated', 'sparse', '0'), exist_ok=True)
...
def run_sparse_reconstruction(base_folder, poses_path, image_dir, db_path):
    ...
    run_colmap_cmd([
        ...
        "--output_path", triangulated_sparse_folder
    ])

    # Bundle Adjustment (Pose 고정)
    run_colmap_cmd([
        "bundle_adjuster",
        "--input_path", triangulated_sparse_folder,
        "--output_path", triangulated_sparse_folder
    ])
