...
{
    mCurrentColorImage = im.clone();  // 원본 컬러 이미지 저장
    mImGray = im;
    if(mImGray.channels()==3)
    {
        if(mbRGB)
    ...
    if (mSensor == System::MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(true, mCurrentColorImage, mImGray,timestamp,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
        else
            mCurrentFrame = Frame(true, mCurrentColorImage, mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
    }
...
void Tracking::CreateNewKeyFrame()
{
    ...
    pKF->imRGB = mCurrentFrame.imRGB.clone();    // ✅ 컬러 이미지 저장
    // std::cout << "pKF->imRGB.size(): " << pKF->imRGB.size() << std::endl;
    // }
    if (mCurrentColorImage.empty()) {
        std::cerr << "[⚠️경고] KeyFrame 생성 시 mCurrentColorImage가 비어 있음! Frame ID: "
